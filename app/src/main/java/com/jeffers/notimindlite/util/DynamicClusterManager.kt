...
/**
 * Represents a node in the semantic clustering hierarchy.
 */
...
object DynamicClusterManager {
    ...
    fun initialize(context: Context) {
        ...
        synchronized(this) {
            if (isInitialized) return
            try {
                // 1. Populate hierarchical base vocabularies
                for ((domain, subMap) in HIERARCHICAL_VOCABULARY) {
                    ...
                    for ((subCategory, words) in subMap) {
                        ...
                        // Also add sub-category keywords to the parent domain for broad matching
                        domainNode.keywords.addAll(words)
                    }
                }

                // 2. Discover installed applications and assign them hierarchically
                ...
                for (app in apps) {
                    ...
                    if (domain != null && rootClusters.containsKey(domain)) {
                        ...
                        // Attempt to assign to a sub-cluster based on keywords
    ...
    /**
     * Backward compatibility: returns a flat map of all clusters (domain + sub-clusters).
     */
    ...
    /**
     * Retrieves the full hierarchical cluster tree.
     */
    ...
    /**
     * Drill down: gets sub-clusters for a specific domain.
     */
    ...
    /**
     * Finds matching cluster nodes for a given set of query tokens.
     * Returns a map of Domain Name -> Matching SubClusterNodes.
     */
    fun findMatchingClusters(queryTokens: List<String>): Map<String, Set<ClusterNode>> {
        ...
        for (token in queryTokens) {
            ...
            for ((domainName, domainNode) in rootClusters) {
                // Check if token matches domain itself or its keywords
